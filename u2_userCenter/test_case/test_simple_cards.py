...
'''
测试登录后，服务卡片点击
1。仅包括，跳转后相同页面的服务卡片：预约挂号、登录轨迹、生活缴费
'''
...
def test_clear(login_home_app_service):
    ...
    assert login_home_app_service[2].ca.btn_settlement().wait(timeout=2.0)

@deal
def test_fix(login_home_app_service):
    '''查看家电维修'''
    login_home_app_service[2].enter_fix()
    assert login_home_app_service[2].fa.btn_settlement().wait(timeout=2.0)

@deal
def test_install(login_home_app_service):
    '''查看家电安装'''
    login_home_app_service[2].enter_install()
    assert login_home_app_service[2].ia.btn_settlement().wait(timeout=2.0)

@deal
def test_discount(login_home_app_service):
    '''查看家电优惠'''
    login_home_app_service[2].enter_discount()
    assert login_home_app_service[2].gd.btn_get_discount().wait(timeout=2.0)

@deal
def test_more(login_home_app_service):
    '''查看更多服务'''
    login_home_app_service[2].enter_more()
    assert login_home_app_service[2].mas.btn_order().wait(timeout=2.0)
